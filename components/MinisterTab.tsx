...
import { Search, Scroll, BookOpen, ChevronLeft, Award } from './Icons';
...
const MinisterTab: React.FC<MinisterTabProps> = ({ isDetailView, setDetailView, initialSelection, onNavigate }) => {
  ...
  useEffect(() => {
    if (initialSelection && initialSelection.tab === AppTab.MINISTER) {
      const chapter = MINISTERS_HANDBOOK_CHAPTERS.find(c => c.id === String(initialSelection.itemId));
      if (chapter) {
  ...
  const filteredChapters = useMemo(() => {
    ...
    if (!searchLower) return MINISTERS_HANDBOOK_CHAPTERS;
  ...
  const elderResults = useMemo(() => {
    if (!filter || filter.length < 2) return [];
    return ELDERS_HANDBOOK_CHAPTERS.filter(c => c.title.toLowerCase().includes(filter.toLowerCase()) || stripHtml(c.fullText).toLowerCase().includes(filter.toLowerCase()));
  }, [filter]);
    ...
    <div className="flex flex-col h-full max-w-3xl mx-auto w-full">
      ...
      <div className="p-4 pb-24 space-y-6">
        ...
        {elderResults.length > 0 && (
          <div>
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 px-1 flex items-center border-t border-gray-200 pt-6"><Award className="w-3 h-3 mr-2" /> Elder's Handbook ({elderResults.length})</h3>
            <div className="space-y-3">{elderResults.map((c) => <button key={`eld-${c.id}`} onClick={() => onNavigate({ tab: AppTab.ELDER, itemId: c.id, highlightTerm: filter })} className="w-full bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-sm text-left hover:border-blue-300"><span className="font-medium text-gray-900">{c.title}</span>{getSearchSnippet(c.fullText, filter, true)}</button>)}</div>
          </div>
        )}
        ...
        {filteredChapters.length === 0 && manualResults.length === 0 && beliefsResults.length === 0 && elderResults.length === 0 && (
